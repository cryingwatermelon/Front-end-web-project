...
export const loginSchema = z.object({
  username: z.string().min(5, 'The username should be at least 5 characters'),
  password: z.string({ message: 'Not a string' }),
})
...
export const insertPhotoSchema = z.object({
  name: z.string().min(2, 'The name should be at least 2 characters'),
  source: z.string({ message: 'Not a string' }),
...
export const uploadImageFileResultSchema = z.object({
  url: z.string({ message: 'Not a string' }),
})
export const ImageIdParamsSchema = z.object({
  id: z.string().length(10, { message: 'The id should be 10 characters' }),
})

export const keywordParamsSchema = z.object({
  keyword: z.string({ message: 'Not a string' }),
})
...
export const patchImageInfoSchema = z.object({
  name: z.string().min(2, 'The name should be at least 2 characters'),
  tags: z.array(z.string()).min(1, 'The tags should be at least 1 characters'),
  source: z.string({ message: 'Not a string' }),
  category: z.number().min(1),
